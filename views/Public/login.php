<?php /** @var $error string */ /** @var $old_login string */ ?>
<div class="auth-block">
    ...
    <form class="auth-form" action="<?= app()->route->getUrl('/login') ?>" method="post">
        <label class="auth-form__label" for="username">Имя</label>
        <input class="auth-form__input" type="text" name="username" id="username" placeholder="Имя пользователя" value="<?= htmlspecialchars($old_login ?? '') ?>" required>
    ...
    <p class="auth-block__paragraph">Нет аккаунта?
       <a class="auth-block__link" href="<?= app()->route->getUrl('/signup') ?>">Зарегистрироваться</a>
    </p>
</div>
